...
app.use(cors({
    origin: ['http://localhost:5000', 'http://127.0.0.1:5000','null', 'http://localhost:3000', 'http://127.0.0.1:3000'],
...
app.post('/Creer_Ville', (req, res) => {
    ...
    if (!nom || !département) {
        return res.status(400).send('Nom de la ville et département sont requis');
    }
    pool.query('INSERT INTO Ville (nom, pays, departement) VALUES(?,?,?)', [nom, "France",département], (err, result) => {
        if (err) {
            console.error(err);
...
app.post('/Creer_Residence', (req, res) => {
    ...
    pool.query('INSERT INTO Residence (numero, rue, idVille) VALUES(?,?,?)', [numero, rue, idVille], (err, result) => {
        if (err) {
            ...
            res.send('Résidence ajouté avec succès!')
        }
    })
});
...
app.delete('/sup_Resident_mail/:mail', (req, res) => {
    ...
    pool.query('DELETE FROM Resident WHERE mail = ?', [mail], (err, result) => {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).send('Aucun résident trouvé avec cet email');
        }
...
app.put('/modif_Resident_age/:id', (req, res) => {
    ...
    pool.query('UPDATE Resident SET age = ? WHERE id = ?', [age, id], 
        (err, result) => {
            if (err) {
                console.error(err);
                return res.status(500).send('Erreur serveur');
...
app.put('/modif_Resident_genre/:id', (req, res) => {
    ...
    pool.query('UPDATE Resident SET genre = ? WHERE id = ?', [genre, id], 
        (err, result) => {
            if (err) {
                console.error(err);
                return res.status(500).send('Erreur serveur');
